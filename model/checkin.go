...
type CheckinRecord struct {
	...
	QuotaRaw     string `json:"quota_awarded_raw"`
}
...
func UserCheckin(userId int) (*Checkin, error) {
	...
	// 计算随机额度奖励
	quotaAwarded := setting.MinChargeQuota
	if setting.MaxChargeQuota > setting.MinChargeQuota {
		quotaAwarded = setting.MinChargeQuota + rand.Int63n(setting.MaxChargeQuota-setting.MinChargeQuota+1)
	}
...
func userCheckinWithTransaction(checkin *Checkin, userId int, quotaAwarded int64) (*Checkin, error) {
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// 步骤2: 在事务中增加用户额度
		if err := updateUserQuotaFieldDeltaTx(tx, userId, "quota", quotaAwarded); err != nil {
...
func GetUserCheckinStats(userId int, month string) (map[string]any, error) {
	...
	for i, r := range records {
		checkinRecords[i] = CheckinRecord{
			...
			QuotaRaw:     strconv.FormatInt(r.QuotaAwarded, 10),
		}
	}
	...
	return map[string]any{
		...
		"total_quota_raw":  strconv.FormatInt(totalQuota, 10),
