...
- 本清单共映射 99 个自研提交。其中 `d81c294f` 是对分组可见性回归测试的泛化
  ...
  或发布能力。
- 唯一已批准退休的本地提交是 `cd2f8814`：它只服务于已退休的 Classic 前端构建，
  不得恢复 `web/classic` 或 `web/default`。现行单前端必须全部位于 `web/src`。
...
2. 对第 4 节每个编号记录最终代码位置、状态和验证结果；先处理高风险项，再处理
   普通前端展示项。
3. 对第 5 节的提交映射做集合比对：每一个提交都必须恰好归属一个保护项或唯一的
   Classic 退休项。
...
- 来源提交：`bca83882`、`693494a0`、`8aaede90`、`9773de9e`。
...
除 `P-02` 的 Classic 退休项外，每一个提交均须保留其对应的功能契约。
...
| P-21 | `bca83882`, `693494a0`, `8aaede90`, `9773de9e` |
