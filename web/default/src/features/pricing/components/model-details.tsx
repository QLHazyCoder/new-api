...
function normalizeCatalogItems(items?: string | readonly string[]): string[] {
  if (!items) return []
  const values = typeof items === 'string' ? items.split(',') : items
  return values
      ...
      <Tabs defaultValue='overview' className='gap-4'>
        <TabsList
          className={cn(
            ...
            perfMetricsVisible ? 'grid-cols-3' : 'grid-cols-2'
          )}
        >
          {TAB_VALUES.filter(
            (value) => perfMetricsVisible || value !== 'performance'
          ).map((value) => {
